...
Figures~\ref{fig:mut-minimize} through~\ref{fig:mut-splice} illustrate each mutation operator using SQL derivation tree examples.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/mutations/fig_3_mutation_minimize.pdf}
\caption{Subtree minimization. The \texttt{Expr} subtree \texttt{a + b} is replaced with the smallest valid derivation of the same non-terminal (\texttt{Expr} $\to$ \texttt{Col} $\to$ \texttt{a}), producing a shorter input that preserves the same coverage bits.}
...
\caption{Deterministic rule substitution (\texttt{mut\_rules}). The targeted \texttt{Expr} node is systematically replaced with every alternative production rule for that non-terminal, generating one mutant per rule.}
\label{fig:mut-rules}
\end{figure}
...
\caption{Random subtree regeneration (\texttt{mut\_random}). A randomly selected node's subtree is replaced with a fresh derivation from the grammar. Here, the comparison \texttt{a > 1} is replaced with a subquery \texttt{a IN (SELECT b FROM q)}.}
...
\caption{Random recursion expansion (\texttt{mut\_random\_recursion}). A recursive production (\texttt{Expr} $\to$ \texttt{Expr + Expr}) is repeated $2^n$ times, increasing expression nesting depth. This mutation only expands; collapsing is performed by minimization.}
...
\caption{Splice mutation (\texttt{mut\_splice}). A subtree from the ChunkStore (populated by minimized interesting inputs) replaces a compatible node in the current tree. The ChunkStore matches by non-terminal: here, an \texttt{Expr} node producing \texttt{a > 1} is replaced with an \texttt{Expr} node producing \texttt{printf('\%.*g', 2147483647, 0.01)}.}
...
\begin{table}[htbp]
\caption{DBMS-Nautilus system components, grouped into five functional subsystems with implementation sizes. Components (1--12) correspond to Figure~\ref{fig:architecture}. The Grammar column refers to the grammar specification loaded at startup.}
...
\midrule
Grammar Spec.\ (1) & Python & \texttt{grammars/} & 788 & 514 weighted production rules \\
