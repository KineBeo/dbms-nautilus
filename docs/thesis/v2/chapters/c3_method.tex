\chapter{DBMS-Nautilus: Design and Implementation}
\label{chap:method}

This chapter presents the design and implementation of DBMS-Nautilus, a grammar-based greybox fuzzing system for SQLite vulnerability detection. The system extends Nautilus~\cite{nautilus} with a structured attack grammar built around the structural primitives methodology and a triage pipeline for automated crash analysis. Section~\ref{sec:overview} provides an architectural overview of the system components and their interactions. Section~\ref{sec:primitives-philosophy} introduces the zero-PoC-contamination design philosophy underlying the grammar. Section~\ref{sec:layer-decomposition} details the two-layer grammar architecture and its concrete production rules. Section~\ref{sec:grammar-evolution} describes the iterative grammar evolution process that constitutes the core contribution of this work. Section~\ref{sec:harness} explains harness construction and compilation. Section~\ref{sec:oracle} defines the oracle classification scheme. Section~\ref{sec:triage} covers the post-campaign triage pipeline.
...
DBMS-Nautilus consists of five major components that together form a closed-loop fuzzing pipeline.

\paragraph{Grammar Engine.}
The Grammar Engine, implemented in Rust (2,441 lines of code in the \texttt{grammartec/} module), loads grammar specifications from Python files via the PyO3 foreign function interface. It builds a rule table from the parsed grammar, assigns weights to each production rule, and provides weighted sampling via the \texttt{loaded\_dice} crate~\cite{loadeddice}, which implements Vose's alias method for $O(1)$ sampling from weighted distributions. Given $K$ rules with weights $w_1, \ldots, w_K$, the alias method normalizes the weights into probabilities $p_i = w_i / \sum_{j=1}^{K} w_j$, constructs a probability table and an alias table, and then produces each sample in constant time using one uniform random number and one comparison. This constant-time sampling is critical because the fuzzer performs on the order of $10^6$ to $10^7$ rule selections per hour-long campaign. The engine supports three mutation operators: random subtree regeneration, recursive expansion of non-terminals, and splice mutations that graft subtrees between different inputs in the queue.

\paragraph{Fuzzer Coordinator.}
The Fuzzer Coordinator (1,658 lines of code in \texttt{fuzzer/}) orchestrates the main fuzzing loop. On each iteration, it either generates a fresh input from the grammar or selects an existing input from the queue and applies a mutation. The resulting parse tree is unparsed into a SQL string, which is then transmitted to the fork server for execution. Upon receiving coverage feedback, the coordinator determines whether the input triggered new edge transitions in the coverage bitmap. If so, it adds the input to the queue for further mutation, building a corpus that progressively explores deeper program behavior.

\paragraph{Fork Server.}
...
Figure~\ref{fig:architecture} illustrates the data flow through the system. The grammar rules define the space of possible SQL inputs. Weighted sampling selects production rules according to their assigned weights. The selected rules drive tree generation, producing a parse tree that is then unparsed into a concrete SQL string. The fork server transmits this string to the harness, which executes it against an instrumented SQLite build. The resulting coverage bitmap is checked for new edges. If new edges are found, the input is retained in the queue, which guides subsequent generation toward unexplored program regions.
