...
Section~\ref{sec:overview} provides a high-level overview of the system and its components. Section~\ref{sec:grammar-design} presents the grammar design methodology, which constitutes the core contribution of this work. The grammar contains 526 production rules organized into a two-layer architecture, designed through root-cause analysis of known CVEs and guided by experimental feedback from fuzzing campaigns.
...
\begin{table}[htbp]
\caption{DBMS-Nautilus system components, grouped into five functional subsystems with implementation sizes. Components (1--12) correspond to Figure~\ref{fig:architecture}. The Grammar Spec.\ row refers to the grammar specification loaded at startup.}
...
\midrule
Grammar Spec.\ (1) & Python & \texttt{grammars/} & 840 & 526 weighted production rules \\
...
The tension between constraints (b) and (c) is the central design challenge. Too few non-terminals and the fuzzer lacks the vocabulary to reach target vulnerabilities. Too many non-terminals and the input space becomes so large that the probability of assembling the right combination within a finite campaign drops below a useful threshold. The grammar addresses this tension through iterative refinement guided by experimental feedback. In its final form, the grammar contains 526 production rules: 476 in Layer~1 (SQL atoms) and 50 in Layer~2 (composed vulnerability-targeting shapes across the four pattern categories). The complete grammar specification is available in the project repository as a Python script.
...
Table~\ref{tab:cve-structural} maps each target CVE to the structural patterns required to trigger it. The table reveals that no structural pattern is unique to a single CVE, and every CVE requires between two and five patterns drawn from different non-terminal groups. Generated columns participate in two CVEs; JOIN patterns serve three; \texttt{coalesce} appears in two. This sharing means that improving the grammar's ability to reach one CVE simultaneously improves its ability to reach others --- a multiplicative benefit that would not arise if each CVE were targeted by isolated grammar rules.
