...
The tension between constraints (b) and (c) is the central design challenge. Too few non-terminals and the fuzzer lacks the vocabulary to reach target vulnerabilities. Too many non-terminals and the input space becomes so large that the probability of assembling the right combination within a finite campaign drops below a useful threshold. The grammar addresses this tension through iterative refinement guided by experimental feedback across four versions. In its final form (v3.3), the grammar contains 520 production rules: approximately 350 in Layer~1 (SQL atoms) and 170 in Layer~2 (composed shapes across the four pattern categories). The complete grammar specification is available in the project repository as a Python script.
