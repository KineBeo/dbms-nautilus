...
The system consists of twelve components organized into a closed feedback loop, illustrated in Figure~\ref{fig:architecture}. Components~1 through~10 form the fuzzer core: the grammar (1) defines the input space, the context engine (2) performs weighted rule selection, tree generation (3) constructs derivation trees, the mutation engine (4) applies structural mutations, minimization (5) shrinks inputs while preserving coverage, the chunk store (6) indexes subtrees for splice mutations, the queue (7) manages inputs through a three-state pipeline, the fork server (8) executes each input via the AFL protocol, the fuzzer executor (9) coordinates per-thread execution, and shared state (10) tracks global coverage bitmaps. Components~11 (instrumented SQLite) and~12 (triage pipeline) are external to the fuzzer core. The following paragraphs describe the key subsystems.
