...
Section~\ref{sec:overview} provides a high-level overview of the system and its components. Section~\ref{sec:grammar-design} presents the grammar design methodology, which constitutes the core contribution of this work. The grammar was iteratively refined across four versions (v3.0 through v3.3, growing from 449 to 520 production rules), guided by root-cause analysis of known CVEs and experimental feedback from fuzzing campaigns.
...
The tension between constraints (b) and (c) is the central design challenge. Too few non-terminals and the fuzzer lacks the vocabulary to reach target vulnerabilities. Too many non-terminals and the input space becomes so large that the probability of assembling the right combination within a finite campaign drops below a useful threshold. The grammar addresses this tension through iterative refinement guided by experimental feedback across four versions.
...
The \texttt{Stress-Query} non-terminal provides eight query patterns, each designed to exercise a distinct path through SQLite's query planner and execution engine. These include basic SELECT with WHERE filtering (Q1), EXISTS subqueries (Q2), NATURAL JOINs (Q3), correlated subqueries (Q4), compound queries with INTERSECT/EXCEPT (Q5), self-JOINs (Q6), window functions with OVER clauses (Q7), and aggregate queries with GROUP BY and HAVING (Q8). Listing~\ref{lst:stress-query} shows four representative alternatives; the remaining patterns follow the same structure.

Query stress patterns target the query planner because this subsystem performs the most complex transformations on the SQL AST: join reordering, subquery flattening, compound query optimization, and window function frame computation. Each transformation involves pointer manipulation and column name resolution across multiple table scopes --- operations where off-by-one errors, null dereferences, and use-after-free bugs frequently occur. NATURAL JOIN (Q3) is particularly effective because it forces the column matching algorithm to pair columns across tables by name, stressing the name resolution logic that has been implicated in multiple CVEs~\cite{cve202013435}. Compound queries (Q5) exercise the INTERSECT/EXCEPT code path, which must align result columns across independently optimized subqueries~\cite{cve202015358, cve202013871}.
...
The \texttt{Boundary-Func-Call} non-terminal targets integer overflow, buffer boundary, and memory allocation edge cases by combining SQLite built-in functions with boundary-value arguments. The \texttt{printf} alternative pairs format specifiers with INT32/INT64 boundary integers, targeting overflow paths in SQLite's string formatting implementation~\cite{cve202013434}. The \texttt{substr} alternative exercises heap buffer boundaries, while \texttt{hex(zeroblob(N))} with large $N$ stresses memory allocation limits. Listing~\ref{lst:boundary-func} shows the alternatives.
